...
import { formatPretty, fullName, shiftDays, todayISO } from "@/lib/data";
import {
  useDeleteSalesSheet,
...
export default function SalesUploadsPage() {
  ...
  // A team head (e.g. Justina, who manages the Sales team) can upload the
  // calling Excel ON BEHALF OF any member of their managed department.
  const isTeamHead = !!me?.is_team_head;
  ...
  const { data: teamMembers = [] } = useEmployees(
    ...
    { enabled: isTeamHead && !!managedDeptSlug },
  );
  // Members the team head can file for — exclude themselves (they'd use the
  // regular self-upload path, and they usually sit in a separate "head" dept).
  const uploadTargets = useMemo(
    () => teamMembers.filter((e) => e.id !== me?.id),
    [teamMembers, me],
  );
  const canUpload =
    !isTarini &&
    (isHR ||
      isTeamHead ||
      ["insideSales", "salesService", "sales"].includes(me?.department?.slug));
  ...
  const [targetId, setTargetId] = useState(""); // team head: whose sheet this is
  const [expandedId, setExpandedId] = useState(null);

  // Team heads must choose a team member; everyone else uploads as themselves.
  const needsTarget = isTeamHead && !isHR;
  ...
  async function submitUpload(e) {
    ...
    try {
      await upload.mutateAsync({
        ...
        note,
        on_behalf_of: targetId ? Number(targetId) : undefined,
      });
      setFile(null);
      setNote("");
    ...
    <div className="space-y-4">
        ...
        <div className="relative flex flex-wrap items-center gap-3">
          ...
          <div className="leading-tight">
            ...
            <p className="text-[11px] text-zinc-600">
              Upload the weekly / monthly calling Excel.{" "}
              {isHR
                ? "HR sees every employee's uploads."
                : needsTarget
                  ? "You can upload sheets for any member of your team."
                  : "Only you and HR can see your uploads."}
            </p>
          </div>
        </div>
      </header>
      ...
      {canUpload && (
        <form onSubmit={submitUpload} className="rounded-lg border border-zinc-200 bg-white p-4">
          <h2 className="text-sm font-semibold text-zinc-900">
            {needsTarget ? "Upload a team member's sheet" : "Upload new sheet"}
          </h2>
            ...
            {needsTarget && (
              <div className="sm:col-span-2 lg:col-span-5">
                ...
                >
                  ...
                  {uploadTargets.map((emp) => (
                    <option key={emp.id} value={emp.id}>
                      {fullName(emp)}{emp.title ? ` · ${emp.title}` : ""}
                    </option>
                  ))}
                </select>
                <p className="mt-1 text-[10px] text-zinc-500">
                  The sheet will be saved under this employee&rsquo;s name.
                </p>
              </div>
            )}
          ...
          <div className="mt-3 flex items-center justify-end gap-2">
            <button
              type="submit"
              disabled={!file || upload.isPending || (needsTarget && !targetId)}
      ...
      <div className="rounded-lg border border-zinc-200 bg-white">
        ...
        <div className="flex flex-wrap items-center gap-2 border-b border-zinc-100 px-4 py-2.5">
          <h2 className="text-sm font-semibold text-zinc-900">
            {isHR || isTarini ? "All uploads" : needsTarget ? "Team uploads" : "My uploads"}{" "}
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            ...
            {/* Employee filter — HR / Tarini / team heads see multiple people */}
            {(isHR || isTarini || needsTarget) && empOptions.length > 1 && (
              <select
                value={empFilter}
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 text-[11px] text-zinc-600">
                      {(isHR || needsTarget) && u.user_name && <span className="font-medium text-zinc-800">{u.user_name} · </span>}
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {(isHR || needsTarget || u.user_id === me.id) && (
                      <button
